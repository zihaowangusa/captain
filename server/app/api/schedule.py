...
async def modify_schedule_event(data:EventData, db=Depends(get_db)):
    ...
    for event in data.events:
        id = event.get("_id", None)
        previous_event = await schedule_collection.find_one({"_id": id})
...
async def delete_schedule_event(data: EventIds, db=Depends(get_db)):
    ...
    if len(data.ids)>0:
        await schedule_collection.delete_many({"_id": { "$in": list(map(lambda id: ObjectId(id), data.ids))}})
